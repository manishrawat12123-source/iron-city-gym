...
const Dashboard = ({ user, onUpgrade }) => {
  ...
  useEffect(() => {
    ...
    if (user?.email) {
      fetch(`http://localhost:5000/api/user/diet-plan?email=${user.email}`)
        .then(res => res.json())
        .then(data => {
      ...
      fetch(`http://localhost:5000/api/user/progress?email=${user.email}`)
        .then(res => res.json())
        .then(data => {
          if (data.success) {
            // Get last 7 days
      ...
      fetch(`http://localhost:5000/api/user/workout-plan?email=${user.email}`)
        .then(res => res.json())
        .then(data => {
          if (data.success) {
  ...
  const handleComplete = async (exerciseId) => {
    try {
      const res = await fetch('http://localhost:5000/api/user/workout/complete', {
        method: 'POST',
